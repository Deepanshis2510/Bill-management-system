...
if (isset($_GET['download_csv'])) {
    ...
    fputcsv($output, ['ID', 'Bill No', 'Party Name', 'GST No', 'Bill Date', 'Taxable Amount', 'Tax Amount', 'Invoice Amount', 'Status']);
...
<body>
    <div class="container">
        ...
        <div class="filter-container">
            <form method="GET" action="super_report_bill.php">
                <label for="status">Status:</label>
                <select name="status" id="status">
                    <option value="">All</option>
                    <option value="pending" <?= ($status_filter == 'pending') ? 'selected' : ''; ?>>Pending</option>
                    <option value="rejected" <?= ($status_filter == 'rejected') ? 'selected' : ''; ?>>Rejected</option>
                    <option value="Forward To Final Approved" <?= ($status_filter == 'Final_approved') ? 'selected' : ''; ?>>Forward For Final Approval</option>
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($bills)): ?>
                    <tr>
                        <td colspan="14" class="no-bills">No bills found.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($bills as $bill): ?>
                        <tr>
                            <!-- <td><?= htmlspecialchars($bill['id']); ?></td> -->
                            <td><?= htmlspecialchars($bill['party_name']); ?></td>
                            <td><?= htmlspecialchars($bill['manual_party_name']); ?></td>
                            <td><?= htmlspecialchars($bill['party_type']); ?></td>
                            <td><?= htmlspecialchars($bill['bill_no']); ?></td>
                            <td><?= htmlspecialchars($bill['bill_type']); ?></td>
                            <td><?= htmlspecialchars($bill['gst_no']); ?></td>
                            <td><?= date('d/m/Y', strtotime($bill['bill_date'])); ?></td> <!-- Date in dd/mm/yyyy format -->
                            <td><?= htmlspecialchars($bill['taxable_amount']); ?></td>
                            <td><?= htmlspecialchars($bill['tax_amount']); ?></td>
                            <td><?= htmlspecialchars($bill['invoice_amount']); ?></td>
                            <td><?= htmlspecialchars($bill['description']); ?></td>
                            <td><?= htmlspecialchars($bill['status']); ?></td>
                            <td>
                                <?php if (!empty($bill['bill_upload'])): ?>
                                    <a href="<?= htmlspecialchars($bill['bill_upload']); ?>" target="_blank">View</a>
                                <?php else: ?>
                                    N/A
                                <?php endif; ?>
                            </td>
