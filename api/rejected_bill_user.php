...
// Initialize filters
$status_filter = isset($_GET['status']) ? $_GET['status'] : 'rejected';  // Default to 'rejected'
...
$sql = "SELECT bills.*, master_data.party_name 
        ...
        WHERE bills.status = :status";  // Only fetch bills with the 'rejected' status

// Apply filters based on user input (if provided)
if (!empty($from_date)) {
...
// Bind parameters for status and date filters
...
<body>
    <div class="container">
                ...
                <tbody>
                    <?php foreach ($bills as $bill) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="view_bill_details.php?id=<?= $bill['id']; ?>" class="view-btn">View</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <?php else : ?>
            <div class="no-bills">No bills found matching the criteria.</div>
        <?php endif; ?>

        <a href="../user_interface.php" class="back-button">Back to Dashboard</a>
    </div>
</body>
</html>
